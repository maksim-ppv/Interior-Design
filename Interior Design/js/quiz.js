...
const PresentSliderActive = (number) => {
    ...
    $(".present-item-left__content").eq(number).fadeIn(500);
...
const contentHeaderButton = document.querySelector('.content-header__button');
const presents = document.querySelector('.presents');



contentHeaderButton.addEventListener('click', e=>{
    popupOpenquiz(presents);
    e.preventDefault();
...
if (popupCloses.length > 0) {
	for (let index = 0; index < popupCloses.length; index++) {
		...
		el.addEventListener('click', function (e) {
			popupClosequiz(el.closest('.presents'));
			e.preventDefault();
		});
	}
}

function popupOpenquiz(curentPopup) {
	if (curentPopup) {
		const popupActive = document.querySelector('.presents_active');
		if (popupActive) {
			popupClosequiz(popupActive);
		}
		curentPopup.classList.add('presents_active');
		curentPopup.addEventListener("click", function (e) {
			if (!e.target.closest('.presents__body')) {
				popupClosequiz(e.target.closest('.presents'));
			}
		});
	}
}

function popupClosequiz(popupActive) {
		popupActive.classList.remove('presents_active');
}
...
const quizArrows = document.querySelector('.quiz__arrows');
const footer = document.querySelector('.footer');

quizArrows.addEventListener('click', (e)=>{
    quizArrows.classList.toggle('quiz-arrows_active');
